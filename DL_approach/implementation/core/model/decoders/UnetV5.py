...
class Decoder(BaseDecoder):
    def __init__(
        ...
        self.feature_pyramid = torch.nn.ModuleList([FeaturePyramid(s,in_ch,out_channels[-1]) for s,in_ch in zip((3,2,1,0),[encoder_channels[0]]+out_channels)])
    ...
    def forward(self, *features):
        ...
        for i, block in enumerate(self.blocks):
            ...
            output += self.feature_pyramid[i+1](x)
        
        return output
    
class FeaturePyramid(torch.nn.Module):
    def __init__(self, stage, in_channel, out_channel):
        super().__init__()
        self.scale = 2**stage
