...
class FeaturePyramid(torch.nn.Module):
    def __init__(self, stage, in_channel, skip_channel, reduction, out_channel):
        ...
        self.q = torch.nn.Parameter(torch.nn.init.normal_(torch.empty(qk_dim,1,1)),requires_grad=True)
        self.qk_dim = qk_dim
    ...
    def forward(self, x, skip):
        ...
        x_score, skip_score = torch.split(self.softmax(torch.concat([x_score,skip_score],dim=1)),[1,1],dim=1)
        
        x = self.x_proj(x)
        skip = self.skip_proj(skip)
        
        out = x*x_score + skip*skip_score
