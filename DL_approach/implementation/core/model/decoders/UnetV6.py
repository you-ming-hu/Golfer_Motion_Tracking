    ...
    def __init__(
        ...
        self.feature_pyramid = torch.nn.ModuleList(
            [FeaturePyramid(s,in_ch,sk_ch,out_channels[-1])
             for s,in_ch,sk_ch in zip((3,2,1,0),[encoder_channels[0]]+out_channels,[None]+encoder_channels[1:])])
...
class FeaturePyramid(torch.nn.Module):
    def __init__(self, stage, in_channel, skip_channel, out_channel):
        ...
        if skip_channel == in_channel:
            self.skip_proj = self.x_proj
        elif skip_channel == out_channel:
            self.skip_proj = torch.nn.Identity()
        elif skip_channel is None:
            self.skip_proj = None
        else:
